...
public interface AuthMapper {
    ...
    // --- toDto (Entity / Principal -> Response DTO) ---

    @Mapping(target = "token", source = "token")
    @Mapping(target = "expiresInSeconds", source = "expiresInSeconds")
    @Mapping(target = "email", source = "user.email")
    @Mapping(target = "fullName", source = "user.fullName")
    CandidateLoginResponseDto toDto(AppUser user, String token, long expiresInSeconds);

    @Mapping(target = "token", source = "token")
    @Mapping(target = "expiresInSeconds", source = "expiresInSeconds")
    @Mapping(target = "email", source = "principal.email")
    @Mapping(target = "fullName", source = "principal.fullName")
    CandidateLoginResponseDto toDto(CustomUserDetails principal, String token, long expiresInSeconds);
    ...
    @Mapping(target = "email", source = "principal.email")
    @Mapping(target = "fullName", source = "principal.fullName")
    StaffLoginResponseDto toDto(CustomUserDetails principal, String token, String roleCode, long expiresInSeconds);
}